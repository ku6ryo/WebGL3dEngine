...
export class BasicProgram extends Program {
  ...
  draw(camera: Camera, thing: Thing, directionalLights: DirectionalLight[]) {
    ...
    gl.uniformMatrix4fv(
      this.getUniformLocation(BasicProgramUniform.MVP_MATRIX),
      false,
      uMvpMatrix
    );
    if (this.#options.useModelInvertMatrix) {
      gl.uniformMatrix4fv(
        this.getUniformLocation(BasicProgramUniform.MI_MATRIX),
        false,
        uMiMatrix
      );
    }
    ...
    if (this.#options.useDirectionalLights) {
      ...
      gl.uniform4fv(
        this.getUniformLocation(BasicProgramUniform.DIRECTIONAL_LIGHTS),
        diLights
      );
    }
